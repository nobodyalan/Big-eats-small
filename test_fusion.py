...
门控残差融合架构冒烟测试(不训练,只验证架构)
  ① 初始恒等性: gate≈0 + adapter2 零初始化 → 旁路输出必须精确为 0
...
print("加载模型 ...")
tokenizer = AutoTokenizer.from_pretrained(resolve_model_path(cfg.model_small_id, cfg.model_small_local))
...
print(f"gate = sigmoid(gate_logit) = {float(torch.sigmoid(fusion.gate_logit)):.6f} (应 ≈ 0.000045)")
